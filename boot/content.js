...
const MSG_HIDE_OVERLAY = "MSG_HIDE_OVERLAY";
...
function getAnalyticsInstance() {
	...
	async function logBulkMsgs(mobile,password,from,to,msg,sentstate) {
		console.log(`${BULK_MSG_URL}?mobile=${mobile}&password=${password}&from=${from}&to=${to}&msg=${msg}&sentstate=${sentstate}&json=1`);
...
///////////////////////////////////////////////////
///////////////////////////////////////////////////
const PAGE_READY_TIMEOUT = 1e3;
const VERIFY_PHONENUMBER_TIMEOUT = 1e3;
...
function initContentScript() {
	...
	if (window.location.href === "https://web.whatsapp.com/" || window.location.href === "https://web.whatsapp.com/") {
		chrome.runtime.onMessage.addListener(msgListener)
	}
}
async function msgListener(msg, sender, response) {
	console.log(msg);
	switch (msg.subject) {
		case MSG_SEND_MESSAGE:
			{
				console.log(msg);sendMessageToNumbers(msg.data.authid,msg.data.authPassword,msg.data.numbers, msg.data.message, msg.data.delay, msg.data.media)
			}
			break;
		case MSG_SEND_MEDIA_MESSAGE:
			{
				console.log(msg);sendAttachMessageToNumbers(msg.data.numbers, msg.data.message, msg.data.media, msg.data.delay);
		...
		case MSG_GET_CONTACTS:
			...
			break;
		case MSG_GET_USER_PHONENUMBER:
			{
				response(getUserPhoneNumber())
			}
			break;
		case MSG_HIDE_OVERLAY:
			...
			break;
		case MSG_ADD_MEDIA_MESSAGE:
			{
				//alert('file size is: ' + msg.data.file.target.files[0].size/1024/1024 + "MB");
				addMedia(msg.data.file);
			}
			break;
		case MSG_GET_PAGE_URL:
...
function getUserPhoneNumber() {
	let userPhoneNumber = "";
	const profileImageDiv = document.querySelector("#side header ._3RWII img");//_1WliW
	if (profileImageDiv) {
		const userImagePath = profileImageDiv.getAttribute("src");
		console.log(userImagePath);
		if (userImagePath.match("&u=[0-9]+")) {
			userPhoneNumber = userImagePath.match("&u=[0-9]+")[0].substring(3)
		}
	}
...
async function verifyPhoneNumber(phoneNumber) {
	...
	do {
		verificationStatus = await attempVerifyOTP(otp);
		console.log(`Verification Status:${verificationStatus}`);
		++retries
	} while (!verificationStatus && retries < VERIFY_RETRIES);
	return verificationStatus
}

function attempVerifyOTP(otp) {
	return new Promise(async function (resolve) {
		...
		}, VERIFY_PHONENUMBER_TIMEOUT)
	})
}

function verifyOTP(otpCode) {
	const allMsgNodes = document.querySelectorAll(".vW7d1");
	const lastMsgNode = allMsgNodes[allMsgNodes.length - 1];
	code = lastMsgNode.querySelector(".copyable-text").innerText.trim();
	if (code === otpCode) {
		const blueTickNode = lastMsgNode.querySelector("._32uRw span");
		const blueDataIcon = blueTickNode.getAttribute("data-icon");
		if (blueDataIcon === "msg-dblcheck-ack") {
			return true
		} else {
			return false
		}
...
async function getAllContacts1() {
	//AllContactsDIV = $("#pane-side .X7YrQ")._3RWII").find("img").attr("src");
	AllContactsDIV = $("#pane-side").find("._3RWII");

	//AllContactsDIV.each(function(index,item) {
	  //$( this ).addClass( "foo" );
	  //alert($("#pane-side .X7YrQ").find("._3RWII")[index]);
	  //alert(item.find("img").attr("src"));
	//});
...
async function getAllContacts() {
	...
	scrollContactsToTop();
	do {
		const currentContacts = await fetchNScrollContacts();
		const newContacts = currentContacts.filter(currentContact => currentContact.phoneNumber && currentContact.phoneNumber.length > 0 && currentContact.phoneNumber.match(/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/im)).filter(currentContact => !allContacts.find(allcontact => currentContact.phoneNumber === allcontact.phoneNumber));
		if (newContacts.length > 0) {
			newContacts.forEach(contact => allContacts.push(contact));
			scrollEnd = false
		} else {
			scrollEnd = true
		}
		sendMsgToExtension({
			...
			subject: MSG_UPDATE_CONTACTS,
			data: allContacts
		})
	} while (!scrollEnd);
	scrollContactsToTop();
	console.log(allContacts);
	await storageProvider.saveAllContacts(allContacts);
	sendMsgToExtension({
		from: "category-content",
		subject: MSG_UPDATE_CONTACTS,
		data: allContacts
...
function scrollContactsToTop() {
	const div = document.getElementById("pane-side");
	div.scrollTop = 0
}

function fetchNScrollContacts() {
	return new Promise(resolve => {
		const currentContacts = getContactsAtCurrentScroll();
		const div = document.getElementById("pane-side");
		div.scrollTop = div.scrollTop + 2 * div.offsetHeight;
		setTimeout(function () {
...
function getContactsAtCurrentScroll() {
	const currentContacts = [];
	document.querySelectorAll("#pane-side .X7YrQ").forEach(node => {
		let image = "";
		let name = "";
		let phoneNumber = "";
		if (node.querySelector("img")) {
			image = node.querySelector("img").getAttribute("src");
			let startindex = image.indexOf("&u=");
			let endindex = image.indexOf("%40");
			phoneNumber = image.substring(startindex + 3, endindex)
		}
		if (node.querySelector("._19RFN")) {//_25Ooe
			name = node.querySelector("._19RFN").textContent;//_25Ooe
			if (phoneNumber.length === 0) {
				phoneNumber = name.replace(/ /g, "")
			}
		}
...
async function sendMessageToNumbers(authid,authPassword,numbers, text, delay,media) {
	//showOverlay();
	let x = 0;
	let series = Promise.resolve();
	numbers.filter(number => number.trim().length > 0).forEach(number => {
		series = series.then(() => {
			return sendMessage(authid,authPassword,number, text,media)
		}).then(() => new Promise(resolve => {
			setTimeout(() => resolve(),delay * 1000) //1e3 * 1000
		}));
...
function sendMessage(authid,authPassword,number, text,media) {
	//showOverlay();

	return new Promise((resolve, reject) => {
		resetMsgBox();
		createMessageLink(number, text).then(() => {
			setTimeout(async function () {
				let result = false;
				if (await isNumberValid()) {
					await logBulkMsgs(authid,authPassword,authid,number,text,'OK');
					await clickSendButton(media);
					result = true
				} else {
					console.log(`${number} is invalid`);
					await logBulkMsgs(authid,authPassword,authid,number,text,'1032');
				}
				resolve(result);
				//alert("delay inside sendMessage: " + delay);
			},1e3 );//delay * 1000
		});
...
function createMessageLink(number, text) {
	return new Promise((resolve, reject) => {
		...
		console.log(bulkWhatsappLink);
		if (bulkWhatsappLink) {
			bulkWhatsappLink.setAttribute("href", `https://wa.me/${number}?text=${encodedText}`)
		} else {
			const spanHtml = `<a href="https://wa.me/${number}?text=${encodedText}" id= "blkwhattsapplink"></a>`;
...
function isNumberValid() {
	const result1 = $("._3RiLE");//_1CnF3 fon invalid number  ///_13HPh
	const result2 = $("._13HPh");//_13HPh for error message
	if (result1 && result1.length === 1) {
		//if ($(result[0]).text() === "Phone number shared via url is invalid.OK") {
			$("._2eK7W").click();//._1WZqU.PNlAR
			return false
		//}
	}
...
async function clickSendButton(media) {
	//await findSendButton();
    $('._3u328').select();
	
	console.log('clickSendButton' + media);
    if(media==1){
	...
	var interval = setInterval(function() {
		// get elem
		if (typeof document.getElementsByClassName("_1g8sv")[0] == 'undefined'){ 
			tries = tries + 1;
			if(tries == 80){
			...
			document.querySelector("[data-icon=send-light]").click();
			clearInterval(interval);
		}
    // the rest of the code
	}, 10);

    //$('._3u328').val($("._3u328").val() + String.fromCharCode(e.which));
...
async function findSendButton() {
	if (!document.getElementsByClassName("_3M-N-")[0]) {//_35EW6_3M-N-
		setTimeout(async function () {
			await findSendButton()
		}, 500)
	}
...
function resetMsgBox() {
	if (document.querySelectorAll("[contenteditable=true]")[0]) {
		document.querySelectorAll("[contenteditable=true]")[0].innerHTML = ""
	}
}

function sendAttachMessageToNumbers(numbers, text, media, delay) {
//alert(media);
	let series = Promise.resolve();
	numbers.filter(number => number.trim().length > 0).forEach(number => {
		series = series.then(() => {
			return sendAttachMessage(number, text,media)
		}).then(() => new Promise(resolve => {
...
function sendAttachMessage(number, text, media) {
	return new Promise((resolve, reject) => {
		resetMsgBox();
		createMessageLink(number, text).then(() => {
			setTimeout(async function () {
				let result = false;
				if (await isNumberValid()) {
					//await clickAttachMediaButton(media);
					await clickAttachSendButton();
					result = true
				} else {
					console.log(`${number} is invalid`)
				}
...
async function addMedia(file) {
	//Method 1
	//var addfile = jQuery('<canvas width="400" height="300" id="canvas" tabindex="0"></canvas><input type="file" id="MediaFile">');
	//

	//Method 2 && 4
	var addfile = jQuery('<canvas id="canvas" tabindex="0"></canvas>');
	//


	if($("#canvas").length<1){
		addfile.appendTo(document.body);
	}


	//Method 1
	//document.getElementById("MediaFile").click();
	//$("#MediaFile").on("change", async function (e) {
	//	handleFiles1(e);
	//});
...
async function clickAttachMediaButton(mediaFile) {

	//await findMediaInputButton();
	var result = $("._3HZor._2rI9W");
	alert(result.length);
	result.eq(0).css({"backgroundColor":"black","width":"500px","height":"500px"});
	result.eq(0).show();
...
/*
	var result = $(".Ijb1Q");
	//result.eq(0).find("input");
	alert(result.length);
	if (result && result.length === 1) {
		$("[data-icon=image]")[0].val(mediaFile);
		$("[data-icon=image]")[0].click();
	}else{
		alert("attach image not found");
	}
*/
/*
var holder = document.getElementById('_1_q7u'),
    tests = {
...
async function clickAttachSendButton() {
	await findAttachSendButton();
	if (document.querySelector("[data-icon=send-light]")) {
		document.querySelector("[data-icon=send-light]").click();
	}
}
async function findAttachSendButton() {
	if (!document.getElementsByClassName("_1g8sv")[0]) {
		setTimeout(async function () {
			await findAttachSendButton();
		}, 500)
	}
...
async function clickAttachButton() {
	//if (document.getElementsByClassName("GPmgf")[0]) {
	//	document.getElementsByClassName("GPmgf")[0].click();
	//}
	//const result = $("[data-icon=clip]");
	//var result = $("._3j8Pd");//.find("[role=button]")title="Attach"
	var result = $("._3fktq");//.find("[role=button]")title="Attach"
	//alert(result.length);
	if (result) {
			//result.eq(4).css({ 'color': 'red', 'width': '350px' })
			result.eq(4).trigger({
				type: 'mousedown',
				which: 3
			});
			result.eq(4).mousedown(function(){
				alert("i clicked clip");
			});
			//result.eq(4).click();
			//alert("i clicked clip");
			return false
		...
		alert("clip: " + result.length);
		//result.eq(4).trigger( "click" );
	return true
}


async function findMediaInputButton() {
	if (!document.getElementsByClassName("_1g8sv___")[0]) {
		setTimeout(async function () {
			await findMediaInputButton();
		}, 500)
	}
...
//alert('im here');
//_3z3lc Ijb1Q		data-icon="image"
//_3z3lc Ijb1Q		data-icon="document"
//_3j8Pd GPmgf


function simulateDrop() {
    //var fileInput = document.getElementById('fileInput'),
    var fileInput = document.getElementById('inputUploadMedia'),
...
$("._3j8Pd").eq(4).click(function(){  
	alert("Hey! Classy Thank For Click ");
});

function showOverlay() {
	var overlay = jQuery('<div id="overlay"><div id="txtContent"></div> </div>');
	if($("#overlay").length<1){
		overlay.appendTo(document.body);
		//#overlay {position: fixed; display: none; width: 100%; height: 100%; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(0,0,0,0.3); z-index: 2; cursor: pointer;}
		//#txtContent {position: absolute; top: 50%; left: 50%; font-size: 50px; color: white; transform: translate(-50%,-50%);-ms-transform: translate(-50%,-50%);}
		$("#overlay").css({'position': 'fixed','width': '100%','height': '100%','top': '0','left': '0','right': '0','bottom': '0','background-color': 'rgba(0,0,0,0.3)','z-index': '99999','cursor': 'pointer'});
			
		$("#txtContent").css({'position': 'absolute','top': '50%','left': '50%','font-size': '50px','color': 'white','transform': 'translate(-50%,-50%)','-ms-transform': 'translate(-50%,-50%)'});
		$("#txtContent").html("<center>Do not refresh this page \n الرجاء عدم المساس بنافذة المتصفح وعدم تحديثها\n</center>");
	}
}
...
async function handleFiles2(e) {
		var ctx = document.getElementById('canvas').getContext('2d');
		//var canvasDataUrl = document.getElementById('canvas');
		var img = new Image();
		img.src = e;
		img.onload = function() {
			// scale canvas to image
...
/*
    try {
        ...
        alert('Unable to copy the address' + err );
    }
*/


/*
	var canvasDataUrl = (document.getElementById('canvas')).toDataURL("image/png");
	//canvasDataUrl.select();
...
async function logBulkMsgs(mobile,password,from,to,msg,sentstate) {
	//console.log('https://www.hiwhats.info/API/logBulkMsgs/?mobile=' + mobile + '&password='+ password + '&from=' + from + '&to=' + to + '&msg='+ msg + '&sentstate=' + sentstate + '&json=1');
	await fetch('https://www.hiwhats.info/API/logBulkMsgs/?mobile=' + mobile + '&password='+ password + '&from=' + from + '&to=' + to + '&msg='+ msg + '&sentstate=' + sentstate + '&json=1');
}

async function handleFiles4(file_url) {
