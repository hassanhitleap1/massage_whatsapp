! function (e, t) {
	...
	h.cors = !!Gt && "withCredentials" in Gt, h.ajax = Gt = !!Gt, w.ajaxTransport(function (t) {
		...
		if (h.cors || Gt && !t.crossDomain) return {
			send: function (i, o) {
				var a, s = t.xhr();
				if (s.open(t.type, t.url, t.async, t.phonenumber, t.password), t.xhrFields)
...
const KEY_CURRENT_MSG_MEDIA = "KEY_CURRENT_MSG_MEDIA";
...
function getStorageInstance() {
	...
	function saveCurrentMessageMedia(currentMedia) {
		return setStorage(KEY_CURRENT_MSG_MEDIA, currentMedia)
	}

	function getCurrentMessageMedia() {
		return getStorage(KEY_CURRENT_MSG_MEDIA)
	}
	...
	function delAllKeys() {
		...
		delStorage(KEY_AUTH_DELAY_TIME);
	}
	return {
		...
		getCurrentMessageMedia: getCurrentMessageMedia,
		saveCurrentMessageMedia: saveCurrentMessageMedia,
...
const MSG_ADD_MEDIA_MESSAGE = "MSG_ADD_MEDIA_MESSAGE";
...
///////////////////////////////////////////////////
///////////////////////////////////////////////////
//const TAB_SINGLE_MSG = "singleMessageTab";
const TAB_MULTI_MSG = "bulkMessageTab";
const TAB_INVITE = "inviteUserTab";
const TAB_SIGNIN = "signinUserTab";
const TAB_SIGNUP = "signupUserTab";
const TAB_SHOWWHATSAPP = "showWhatsappTab";
const TAB_GROUPS = "groupsTab";
const TAB_SETTINGS = "settingsTab";
const TAB_ABOUTUS = "aboutUsTab";
const TAB_REPORTS = "reportsTab";
const TAB_LOGOUT = "logOutTab";

let activeTab = TAB_SIGNIN;
...
//let authSignupName = "";
//let authSignupEmail = "";
//let authSignupConfirmPassword = "";
let signinPhoneError = "";
...
async function initApp() {
	chrome.runtime.onMessage.addListener(async function (msg, sender) {
		console.log(msg);
		if (msg.subject === MSG_UPDATE_CONTACTS) {
			contacts = msg.data;
			updateView()
		} else if (msg.subject === MSG_REF_COOKIE_VALUE) {
			refid = msg.data;
			await storageInstance.saveRefID(refid)
		}
	});
	
	authid = await storageInstance.getAuthId();
	authPhoneNumber = await storageInstance.getAuthPhoneNumber();
	authPassword = await storageInstance.getAuthPassword();
	activeTab = await storageInstance.getActiveTab();
	refid = await storageInstance.getRefID();
	DelayTime = await storageInstance.getAuthDelayTime();
	...
	await initSingleMessageTab();
	await initBulkMessageTab();
	await initInvitationTab();

	updateView();
	addEventListeners();
	const installTimeStamp = await storageInstance.getInstallTimeStamp();
	if (installTimeStamp) {
		analyticsInstance.logRevisits(authid)
	} else {
		sendMsgToExtension({
			from: "popup.js",
			subject: MSG_GET_REF_COOKIE
		})
	}
}


async function initSignUpTab() {}
async function initSignInTab() {}
async function initGroupsTab() {}
async function initSettingsTab() {
	$("#settingsDelayTimeLabel").html(DelayTime);
	console.log("DelayTime in init: " + DelayTime);
}

//initSingleMessageTab
...
async function initBulkMessageTab() {
	...
	msgMedia = await storageInstance.getCurrentMessageMedia();

	phoneNumber = await storageInstance.getPhoneNumber();
	authPhoneNumber = await storageInstance.getAuthPhoneNumber();
	authPassword = await storageInstance.getAuthPassword();
	AuthName = await storageInstance.getAuthName();
	AuthKind = await storageInstance.getAuthKind();
	AuthExpDate = await storageInstance.getAuthExpDate();
	DelayTime = await storageInstance.getAuthDelayTime();
	...
	//$("#image_upload_preview").attr('src', msgMedia);
	$("#num_count").html(phoneNumbersBulk.length);
	$("#ucp_name").html(AuthName);
	$("#ucp_kind").html(AuthKind);
	$("#ucp_exp").html(AuthExpDate);

}
//initBulkMessageTab

//initInvitationTab
async function initInvitationTab() {
	const allContacts = await storageInstance.getAllContacts();
	if (allContacts) {
...
async function updateView() {
	...
	if (!isPageWebWhatsapp) {
		activeTab = TAB_SHOWWHATSAPP;
		$(`#${TAB_SIGNIN}`).hide();
		$(`#${TAB_SIGNUP}`).show();
		//$(`#${TAB_SINGLE_MSG}`).hide();
		$(`#${TAB_MULTI_MSG}`).hide();
		$(`#${TAB_INVITE}`).hide();
		$(`#${TAB_SHOWWHATSAPP}`).hide();
	...
	if (activeTab) {
		console.log(`Displaying Active Tab`);
		$(`#${activeTab}`).tab("show");
	} else {
		console.log(`Displaying Signin Tab`);
		$(`#${TAB_SIGNIN}`).tab("show");
	}

	$(`#${TAB_MULTI_MSG}`).click(async function (e) {
		initBulkMessageTab();
	});

	updateSigninView();
	updateSingleMessageView();
...
function updateSettingsView() {
	console.log(`Update DelayTime= ` + DelayTime);
	$("#settingsDelayTimeLabel").html(DelayTime);
}
//updateSettingsView
...
function updateSigninView() {
	...
	$("#signinPhoneInput").val(authPhoneNumber);
	$("#signinPasswordInput").val(authPassword);
	//$("#signinPhoneMsg").html(signinPhoneError);
	//$("#loginFailedMsg").html(loginFailedMsg);
	//$("#loginSuccessMsg").html(loginSuccessMsg)
}
//updateSigninView

//updateInvitationView
function updateInvitationView() {
	let contactsHTML = "";
	if ($("#searchContactTextBox").val().length > 0) {
		matchingContacts.forEach(contact => {
			const isAddedPhoneNumber = invitationNumbers.find(number => number === contact.phoneNumber);
			contactsHTML = contactsHTML + `<div class="${isAddedPhoneNumber?"profile-card-wrappper active":"profile-card-wrappper"}">\n      <div class="overlay"></div>\n      <div class="profile-card">\n          <span class="checkbox"><i class="icon icon-check"></i></span>\n          <img src="${contact.image}">\n          <div class="text">\n              <label>${contact.name}</label>\n              <em class="phnum" dir="ltr">${contact.phoneNumber}</em>\n          </div>\n      </div>\n  </div>`
		})
	} else {
	...
	$("#sendInviteBulk").html(` ارسل الدعوات لـ (${invitationNumbers.length})`);
	if (invitationNumbers.length === 0) {
		$("#sendInviteBulk").prop("disabled", true)
	} else {
		$("#sendInviteBulk").prop("disabled", false)
	}
	$("#contacts-holder").html(contactsHTML);
	$(".profile-card-wrappper").click(function () {
		const clickedNum = $($(this).find(".phnum")[0]).text();
		if(invitationNumbers.includes(clickedNum)){
			removeItem(invitationNumbers, clickedNum);
		}else{
			invitationNumbers.push(clickedNum);
		}
		updateView()
	})
}
...
//updateSingleMessageView
function updateSingleMessageView() {
...
//updateBulkMessageView
async function updateBulkMessageView() {
...
//addEventListenersShowWhatsappView
function addEventListenersShowWhatsappView() {
	$("#showWhatsappBtn").click(async function () {
		sendMsgToExtension({
			from: "popup.js",
			subject: MSG_SHOW_WHATSAPP_TAB,
			data: ""
		})
	})
}
...
//addEventListenersSigninView
async function addEventListenersSigninView() {

	SignInPhoneNumber = await storageInstance.getAuthPhoneNumber();
	$("#signinPhoneInput").val(SignInPhoneNumber);

	SignInPassword = await storageInstance.getAuthPassword();
	$("#signinPasswordInput").val(SignInPassword);

	$("#signinBtn").click(async function (e) {

		SignInPhoneNumber = $("#signinPhoneInput").val();
		SignInPassword = $("#signinPasswordInput").val();

		e.preventDefault();
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getLoginUrl(),
			data: {'mobile': SignInPhoneNumber, 'password': SignInPassword, 'json': 1 },
			dataType: 'json',
			...
			success: async function(data)
			{
				if (data.success) {
					...
					activeTab = TAB_MULTI_MSG;
					authid = SignInPhoneNumber;
					authPhoneNumber = SignInPhoneNumber
					authPassword = SignInPassword;

					await storageInstance.savePhoneNumber(SignInPhoneNumber);
					await storageInstance.saveAuthPhoneNumber(SignInPhoneNumber);
					await storageInstance.saveAuthPassword(SignInPassword);
					await storageInstance.saveAuthId(authid);
					await storageInstance.saveActiveTab(activeTab);
					await storageInstance.saveAuthName(data.msg.fullname);
					await storageInstance.saveAuthKind(data.msg.group);
					await storageInstance.saveAuthExpDate(data.msg.ExpireDate);

					//await analyticsInstance.logLogin(authPhoneNumber, authPassword, refid);
					initBulkMessageTab();
					updateView();
...
//addEventListenersSignUpView
function addEventListenersSignUpView() {

	$("#signupBtn").click(async function (e) {

		signupName = $("#signupNameInput").val();
		signupPhoneNumber = $("#signupCountryCodeInput").val() + parseInt($("#signupMobileInput").val(), 10);
		...
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getSignUpUrl(),
...
//addEventListenerslogOutView
function addEventListenerslogOutView() {
	$("#logOutBtn").click(async function () {
		activeTab = TAB_SIGNIN;
		await storageInstance.delAllKeys();
		await storageInstance.saveActiveTab(activeTab);
		authid = '';
		updateView();
	});
...
//addEventListenersGroupsView
async function addEventListenersGroupsView() {
	
	UpdateGroupsLists();


	$("#groupAddBtn").click(async function (e) {
		...
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getUrl() + '/AddGroup/',
	...
	$("#groupDeleteBtn").click(async function (e) {
		...
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getUrl() + '/DeleteGroup/',
	...
	$("#groupAddNumberBtn").click(async function (e) {
		...
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getUrl() + '/AddNumbers/',
...
//addEventListenersSettingsView
async function addEventListenersSettingsView() {

	phoneNumber = await storageInstance.getPhoneNumber();
	authPhoneNumber = await storageInstance.getAuthPhoneNumber();
	authPassword = await storageInstance.getAuthPassword();

	$("#settingsDelayTimeInput").slider({value:DelayTime});
	$("#settingsDelayTimeInput").on("slide",async function(slideEvt) {
		$("#settingsDelayTimeLabel").text(slideEvt.value);
		await storageInstance.saveAuthDelayTime(slideEvt.value);
	});

	$("#settingsChangePasswordBtn").click(async function (e) {

		settingsOldPassword = $("#settingsOldPasswordInput").val();
		settingsNewPassword = $("#settingsNewPasswordInput").val();
		settingsConfirmPassword = $("#settingsConfirmPasswordInput").val();

		e.preventDefault();
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getUrl() + '/ChangePassword/',
			data: {'mobile': authPhoneNumber, 'password': authPassword, 'oldpassword': settingsOldPassword, 'newpassword': settingsNewPassword, 'confirmpassword': settingsConfirmPassword, 'json': 1 },
			dataType: 'json',
			beforeSend: function(jqXHR, settings) {
				console.log(settings.url);
			},
			...
			{
				if (data.success) {
					$("#settingsSuccessMsg").fadeIn().delay(2000).fadeOut();
				}else{
					$("#settingsFailedMsg").html(showErrorMessage(data.msg)).fadeIn().delay(2000).fadeOut();
				}
			}
		});
...
//addEventListenersReportsView
function addEventListenersReportsView() {
    ...
    $.ajax({
        ...
        url:"AJAX.ShowChart.php?pyear=" + pyear + '&pop=' + pop + '&pmonth=' + pmonth,
        //url:"AJAX.JSONNews.php",
        dataType: 'JSON',
        type: 'POST',
...
//addEventListenersSingleMessageView
function addEventListenersSingleMessageView() {
	$("#sendMessage").click(async function () {
		const phoneNumberToMsg = phoneNumber.trim();
		await analyticsInstance.logSingleMsgs(authid);
		sendMsgToActiveTab({
			from: "home.js",
			...
			data: {
				...
				message: msg,
				numbers: [phoneNumberToMsg],
				delay: delay
			}
		})
	});
	$("#inputPhoneNumber").on("input", async function () {
		phoneNumber = $(this).val();
		await storageInstance.savePhoneNumber(phoneNumber);
		updateView();
	});
	$("#clearPhoneNumberId").click(async function () {
		phoneNumber = "";
		await storageInstance.savePhoneNumber(phoneNumber);
		updateView();
	});
	$("#inputmessage").on("input", async function () {
		msg = $(this).val();
		await storageInstance.saveCurrentMessage(msg);
		updateView()
	});
	$("#searchPhoneNumberId").click(async function () {
		const phoneNumberToMsg =  phoneNumber.trim();
		sendMsgToActiveTab({
			from: "home.js",
			subject: MSG_SEND_MESSAGE,
...
function addEventListenersBulkMessageView() {
	...
	$("#sendMessageBulk").click(async function () {
		const phoneNumbersBulkToSend = phoneNumbersBulk.map(number => {
			return number
		});

		console.log("media click: " + media);
		//await analyticsInstance.logBulkMsgs(authid,authPassword,authid,phoneNumbersBulkToSend,msgBulk);
		AllNumbers = phoneNumbersBulk;
		timer.set({action:function() { SendMsg(msgBulk,phoneNumbersBulkToSend,media);}, time : delay * 1000, autostart : true });
        ...
        if (playActive == 0) {
			//$("#sendMessageBulk").find('i').removeClass('fa fa-play').addClass('fa fa-pause');
			$("#sendMessageBulk").removeClass('btn btn-success').addClass('btn btn-primary');
			$("#sendMessageBulk").html('<i class="fa fa-pause"></i>  ايقاف الارسال');
			playActive = 1;
            timer.play();
        }else {
			//$("#sendMessageBulk").find('i').removeClass('fa fa-pause').addClass('fa fa-play');
			$("#sendMessageBulk").removeClass('btn btn-primary').addClass('btn btn-success');
			$("#sendMessageBulk").html('<i class="fa fa-play fa-rotate-180"></i> بدء الارسال');
			playActive = 0;
            timer.toggle();
		}

		$("#stopMessageBulk").click(function () {
			timer.stop();
		});
	...
	//##############################################################################//
	$("#groupsTableAddBtn").click(async function () {
		var groupid = 0;
		...
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getUrl() + '/GetContactsByGroup/',
			data: {'mobile': authPhoneNumber, 'password': authPassword, 'groupid': groupid, 'json': 1 },
			...
			success: async function (data) {
				...
				//alert(JSON.stringify(phoneNumbersBulk));
				await storageInstance.savePhoneNumbersBulk(phoneNumbersBulk);
				$("#num_count").html(phoneNumbersBulk.length);
				updateBulkMessageView();
			}
	...
	//##############################################################################//
	$("#btnWaImportNumbersButton").click(async function () {
		contacts.forEach(contact => {
			phoneNumbersBulk.push(contact.phoneNumber)
		});
	...
	//##############################################################################//
	$("#btnInsertFileButton").click(async function () {

		//Method 1
		//sendMsgToActiveTab({
		//	from: "home.js",
		//	subject: MSG_ADD_MEDIA_MESSAGE,
		//	data: {
		//	}
		//});
...
//}
		...
		//alert("تم ارفاق الصورة!!! ");
	});
	//##############################################################################//
	$("#inputInsertFile").on("change", async function (e) {
		//const file = $(this)[0].files[0];
		//f = e.target.files[0];
		//f = this.files[0];
		//alert('file size is: ' + f.size/1024/1024 + "MB");
		...
		*/
		...
		if (this.files && this.files[0]) {
			...
			reader.onload = async function (e) {
				...
				var data = {'mobile': authPhoneNumber, 'password': authPassword, 'file': reader.result, 'json': 1 };
				
				$.ajax({
					type: 'POST',
					url : await analyticsInstance.getUrl() + '/UploadFile/',
					data: {'mobile': authPhoneNumber, 'password': authPassword, 'file': reader.result, 'json': 1 },
					beforeSend: function(jqXHR, settings) {
						console.log(settings.url);
					},
					success: async function(response) {
						msgMedia = response.msg;
						$("#image_upload_preview").attr('src', msgMedia);
						await storageInstance.saveCurrentMessageMedia(response.msg);
	...
	//##############################################################################//
	$("#btnUploadFileButton").click(function () {
		document.getElementById("inputUploadMedia").click();
		updateView();
	});
	//##############################################################################//
	$("#inputUploadMedia").on("change", async function (e) {
		authPhoneNumber = await storageInstance.getAuthPhoneNumber();
		authPassword = await storageInstance.getAuthPassword();
	...
	$('#image_upload_preview').click(function () {
		var r = confirm("هل تريد بالتأكيد حذف الصورة المدرجة؟");
		if (r == true) {
			media = 0;
	...
	//##############################################################################//
	async function SendMsg(msg,numbers,media) {
		...
		if(checkUser()){
			...
			if(count <= AllNumbers.length){
				console.log("length: " + AllNumbers.length);
				console.log("count: " + count);

				percent = (100 * count) / AllNumbers.length;
				$("#LabelBulkTimer").html(Math.round(percent));

				$("#ProgressBarMessageBulk")
				.css("width", percent + "%")
				.attr("aria-valuenow", percent)
				.text(percent + "% Complete");
		console.log("media send: " + media);
				...
				//await analyticsInstance.logBulkMsgs(authid,authPassword,authid,numbers[0],msg);

				numbers.shift();
				console.log("numbers 2: " + numbers);

				$("#inputNumbersBulk").val(numbers.join("\n"));
				$("#num_count").html(numbers.length);
				...
				//sendMsgToActiveTab({
				//	from: "home.js",
				//	subject: MSG_HIDE_OVERLAY
				//});
			}
			await storageInstance.savePhoneNumbersBulk(numbers);
		}
	}
	//##############################################################################//
	async function checkUser() {
		...
		$.ajax({
			type: "GET",
			url: await analyticsInstance.getUrl() + '/checkUser/',
			data: {'mobile': authPhoneNumber, 'password': authPassword, 'json': 1 },
			contentType: "application/json",              
			dataType: 'json',
			beforeSend: function(jqXHR, settings) {
				console.log(settings.url);
			...
			success: function (data) {
				...
				if(data.msg.status != 'active'){
					...
					timer.stop();
					alert("هذا الحساب منتهي الصلاحية ... الرجاء شراء باقة");
					return_value = false;
				}else{
					return_value = true;
				}
	...
	async function writeToClipboard(imageBlob) {
		try {
			...
			media = 1;
			alert("تم ادراج الصورة الى الرسالة بنجاح !!!");
		} catch (error) {
			console.log(error);
			media = 0;
			alert("فشل في ادراج الصورة الى الرسالة, يرجى المحاولة مرة أخرى ...");
			$('#image_upload_preview').attr('src', '');
		}
	}
	//##############################################################################//
	async function handleFiles(e) {
		...
		reader.onload = function () {
			...
			$('#image_upload_preview').attr('src', reader.result);
...
//addEventListenersInvitationView
function addEventListenersInvitationView() {
	$("#searchContactTextBox").on("keyup", async function () {
		const value = this.value.trim();
		console.log(this.value);
		matchingContacts = contacts.filter(contact => {
			if (contact.name.toLowerCase().indexOf(value.toLowerCase()) !== -1) {
				return true
			} else {
				return false
			}
	...
	$("a[data-toggle=tab]").click(function () {
		activeTab = this.id;
		storageInstance.saveActiveTab(this.id);
		if (activeTab === TAB_INVITE) {
			if (!contacts || contacts.length === 0) {
				parseAllContactsFromShell()
			}
		}
	});

	$("#sendInviteBulk").click(function () {
		sendMsgToActiveTab({
			from: "home.js",
			subject: MSG_SEND_MESSAGE,
			data: {
...
function parseAllContactsFromShell() {
	sendMsgToActiveTab({
		from: "home.js",
		subject: MSG_GET_CONTACTS
	})
}
...
async function UpdateGroupsLists() {
    ...
    $.ajax({
        type: "GET",
        url: await analyticsInstance.getUrl() + '/GetUserGroups/',
		data: {'mobile': authPhoneNumber, 'password': authPassword, 'json': 1 },
        contentType: "application/json",              
		dataType: 'json',
		beforeSend: function(jqXHR, settings) {
        ...
        success: function (data) {
            $("#groupDeleteList").empty();
            $("#groupAddNumberList").empty();
			var trHTML = '';
            $.each(data, function (index, item) {
				//alert(JSON.stringify(item.name));
                $("#groupDeleteList").append($("<option></option>").val(item.id).html(item.name));
                $("#groupAddNumberList").append($("<option></option>").val(item.id).html(item.name));
				trHTML += '<tr><td><input type="radio" name="groupid" class="radioBtnClass" value="' + item.id + '"></td><td>' + item.name + '</td><td>' + item.total + '</td></tr>';
            });
			$('#groupsTableBodyModal').append(trHTML);
        }
    });
}

function getInvitationMessage() {
	return 'أنا أستخدم برنامج هاي واتساب وانصح باستخدامه \n برنامج يمكنك من ارسال الاف رسائل الواتس اب بكل سهولة\n  يمكنك الحصول على نسخة تجريبة مجانا عن طريق الرابط : \n https://hiwhats.info/ \n نتمنى ان ينال اعجابكم ';
}
